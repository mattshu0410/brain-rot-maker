...
const generateNBAPlayer = () => {
  const names = ["LeBron James", "Stephen Curry", "Kevin Durant", "Giannis Antetokounmpo", "Kawhi Leonard"]
  ...
  return (
    <>
      ...
      <div className="z-10 w-full max-w-5xl items-center justify-between">
        <h1 className="text-4xl font-bold mb-8 text-center">Celebrity Lip-Sync Generator</h1>
        <Input />
        <div className="my-12">
          <h2 className="text-2xl font-semibold mb-6 text-center">Select Top Video</h2>
          <VideoCarousel videos={topVideos} onSlideChange={handleTopVideoChange} />
          <p className="text-center mt-4">
            Selected Top Video:{" "}
            {selectedTopVideo !== null ? topVideos.find((v) => v.id === selectedTopVideo)?.name : "None"}
          </p>
        </div>
        <div className="my-12">
          <h2 className="text-2xl font-semibold mb-6 text-center">Select Celebrity</h2>
          <VideoCarousel videos={celebrityVideos} onSlideChange={setSelectedCelebrity} />
          <p className="text-center mt-4">Selected Celebrity: {selectedCelebrity + 1}</p>
        </div>
